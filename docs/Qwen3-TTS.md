...
Qwen/Qwen3-TTS-12Hz-1.7BCustomVoice

CustomVoice

Predefined speaker voices with optional stylecontrol

Qwen/Qwen3-TTS-12Hz-1.7BVoiceDesign
...
Qwen/Qwen3-TTS-12Hz-0.6BCustomVoice
...
Smaller/faster variant for voice cloning
...
--language : Language. Options: Auto , Chinese , English , Japanese , Korean , German , French , Russian , Portuguese , Spanis h , Italian
...
This endpoint follows the OpenAI Audio Speech API format with additional Qwen3-TTS parameters.
...
{
    "voices": ["aiden", "dylan", "eric", "one_anna", "ryan", "serena", "sohee", "uncle_fu",
"vivian"]
}
...
Model to use (optional, should match server ifspecified)
...
Speaker name (e.g., vivian, ryan, aiden)
...
Audio format: wav, mp3, flac, pcm, aac, opus
...
Playback speed (0.25-4.0, not supportedwith stream=true )
...
Task: CustomVoice, VoiceDesign, or Base
...
Language (see supported languages below)
...
Voice style/emotion instructions
...
Stream raw PCM chunks as they are decoded(requires response_format="pcm" )
...
Reference audio (URL or base64 data URL)
...
Set stream=true with response_format="pcm" to receive raw PCM audio chunks as they are decoded (one chunk per 25-frame Code2Wav window):
...
Constraints: - stream=true requires response_format="pcm" (raw 16-bit signed PCM, 24 kHz mono).  speed adjustment is not supported when streaming. - Requires the server stage config to have async_chunk: true (default in qwen3_tts.yaml ).
...
3. Out of memory: Use smaller model variant ( Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice ) or reduce --gpumemory-utilization
