...
const ANDROID_UPDATE_URL =
    "http://100.114.203.28:8090/proatt/latest-android.json";
...
const loadHistory = async (): Promise<HistoryItem[]> => {
    ...
    if (store) {
        try {
            const history = await store.get<HistoryItem[]>(
...
const analyzeAudioContent = async (blob: Blob): Promise<AudioMetrics> => {
    ...
    try {
        ...
        const noiseFloor =
            sampledAmplitudes
                .slice(0, noiseFloorIndex)
                .reduce((a, b) => a + b, 0) / (noiseFloorIndex || 1);

        // Top 5% is signal
        const signalCeilingIndex = Math.floor(sampledAmplitudes.length * 0.95);
        const signalLevel =
            sampledAmplitudes
                .slice(signalCeilingIndex)
                .reduce((a, b) => a + b, 0) /
        ...
        let clarityScore = Math.min(
            100,
            Math.max(0, Math.log10(snrRatio) * 40),
        );

        // Penalty for silence extremes
        const silenceRatio =
            sampledAmplitudes.filter((s) => s < 0.01).length /
            sampledAmplitudes.length;
        ...
        if (sliceStart > 0 && sliceStart + 2048 < rawData.length) {
            ...
            if (rms > 0.01) {
                // Only calculate pitch if there's signal
                for (let offset = 20; offset < 1000; offset++) {
                    // Check frequencies between ~44Hz and 2200Hz
                    let correlation = 0;
...
export default function App() {
    ...
    const { activePanel, setActivePanel, isTransitioning } =
        useActivePanel("att");
    ...
    const [recordingStyle, setRecordingStyle] =
        useState<RecordingStyle>("Dictation");
    ...
    const [aiModel, setAiModel] = useState<string>(
        () => localStorage.getItem("gemini_ai_model") || "gemini-2.5-pro",
    );
    ...
    const [logs, setLogs] = useState<
        { msg: string; type: "info" | "success" | "error"; time?: Date }[]
    >([]);
    ...
    >(() => {
        return (
            (localStorage.getItem("gemini_outputLanguage") as any) || "English"
        );
    });
    const [outputStyle, setOutputStyle] = useState<OutputStyle>(() => {
        ...
        return (
            (localStorage.getItem("gemini_outputStyle") as any) || "Verbatim"
        );
    });
    ...
    const [transcriptionMode, setTranscriptionMode] =
        useState<TranscriptionMode>(() => {
            return (
                (localStorage.getItem("voice_ai_mode") as TranscriptionMode) ||
                "auto"
            );
        });
    ...
    const [whisperServerUrl, setWhisperServerUrl] = useState<string>(() => {
        return (
            ...
            "http://100.114.203.28:8765"
        );
    });
    const [whisperTestStatus, setWhisperTestStatus] = useState<
    ...
    // TTS Settings
    const [ttsEngine, setTtsEngine] = useState<"piper" | "chatterbox">(
        "chatterbox",
    );
    ...
    // Persist Effects
    useEffect(
        () => localStorage.setItem("gemini_outputLanguage", outputLanguage),
        [outputLanguage],
    );
    useEffect(
        () => localStorage.setItem("gemini_outputStyle", outputStyle),
        [outputStyle],
    );
    useEffect(
        () =>
            localStorage.setItem("gemini_customStylePrompt", customStylePrompt),
        [customStylePrompt],
    );
    useEffect(
    ...
    useEffect(
        () => localStorage.setItem("voice_ai_mode", transcriptionMode),
        [transcriptionMode],
    );
        ...
        const checkSidecar = async () => {
            try {
                ...
                if (health?.status === "healthy") {
                    ...
                    addLog(
                        "Voice AI Sidecar conectado - Transcricao local ativada",
                        "success",
                    );
                } else {
    ...
    useEffect(() => {
        const checkForUpdatesDesktop = async () => {
            try {
                ...
                if (update) {
                    ...
                    console.log(
                        `[Updater] Nova versao disponivel: ${update.version}`,
                    );
                    ...
                    await update.downloadAndInstall((event) => {
                        if (event.event === "Progress") {
                            ...
                            if (data.contentLength && data.contentLength > 0) {
                                const pct = Math.min(
                                    (downloaded / data.contentLength) * 100,
                                    100,
                                );
                    ...
                    console.log(
                        "[Updater] Download concluido. Pronto para reiniciar.",
                    );
        ...
        const checkForUpdatesAndroid = async () => {
            try {
                ...
                if (!resp.ok) {
                    console.log(
                        `[Updater-Android] Servidor retornou ${resp.status}`,
                    );
                    setUpdateStatus("idle");
                    return;
                }
                ...
                console.log(
                    `[Updater-Android] Versao remota: ${remoteVersion}`,
                );
                ...
                if (isNewerVersion(remoteVersion, localVersion)) {
                    ...
                    console.log(
                        `[Updater-Android] Nova versao disponivel: ${remoteVersion}`,
                    );
                    ...
                    console.log(
                        "[Updater-Android] Nenhuma atualizacao disponivel",
                    );
                    setUpdateStatus("idle");
                }
            } catch (e) {
            ...
            if (savedContexts.length > 0) {
                ...
                if (lsPools && lsMemory) {
                    try {
                        ...
                        const memory = JSON.parse(lsMemory) as Record<
                            string,
                            string
                        >;
    ...
    const startRecording = async () => {
        ...
        if (isTauri()) {
            try {
                ...
                const deviceLabel =
                    selectedMicId !== "default" ? selectedMicId : null;
    ...
    const handleDownloadText = async (format: "txt" | "md") => {
        ...
        if (isTauri()) {
            try {
                ...
                addLog(
                    "Exportacao nativa falhou, tentando fallback...",
                    "warning",
                );
            }
        }
            ...
            try {
                ...
                if (filePath) {
                    const bytes = new Uint8Array(
                        await blobToExport.arrayBuffer(),
                    );
    ...
    const processAudio = async () => {
        ...
        try {
            ...
            const buildStylePrompt = (rawText: string): string => {
                const currentMemory =
                    contextMemory[activeContext] || "No previous context.";
          ...
          `;
                    ...
                    if (isPortuguese) {
                        ...
                        styleInstruction =
                            ...
                            `Adapte para o estilo ${outputStyle}.`;
                    } else {
        ...
        `;
            ...
            const refineWithGemini = async (
                ...
                const response = await ai.models.generateContent({
                    model: aiModel,
                    config: {
                        temperature: isPromptEngineeringMode
                            ? 0.2
                            : isVerbatimMode
                              ? 0.1
                              : 0.4,
            ...
            if (useLocalSTT && voiceAIClient.current) {
                ...
                const format = VoiceAIClient.getFormatFromMimeType(
                    audioBlob.type,
                );
                ...
                try {
                    ...
                    if (outputStyle !== "Whisper Only" && currentApiKey) {
                        ...
                        try {
                            finalText = await refineWithGemini(
                                finalText,
                                currentApiKey,
                            );
                    ...
                    const mode =
                        outputStyle !== "Whisper Only" && currentApiKey
                            ? "Whisper + Gemini (text-only)"
                            : "Whisper";
                    ...
                    addLog(
                        `Sidecar falhou: ${sidecarError.message}`,
                        "warning",
                    );
            ...
            >
                <div className="w-full max-w-sm p-8 bg-white/5 border border-white/10 rounded-lg">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="text-[10px] opacity-60 mb-1 block">
                                Usuario
                            </label>
                            <input
                                type="text"
                    ...
                    <PanelATT
                        ...
                        selectedMicLabel={
                            selectedMicId === "default"
                                ? "Default Mic"
                                : availableMics
                                      .find((m) => m.deviceId === selectedMicId)
                                      ?.label?.slice(0, 15)
                        }
                        autoGainControl={autoGainControl}
                    />
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div className="md:col-span-2">
                                        <label className="text-[10px] opacity-60 mb-1.5 block">
                                            Input Device
                                        </label>
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-sm border border-white/5">
                                        ...
                                        >
                                            <div
                                                className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${noiseSuppression ? "left-6" : "left-1"}`}
                                            />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-sm border border-white/5">
                                        ...
                                        >
                                            <div
                                                className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${echoCancellation ? "left-6" : "left-1"}`}
                                            />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-sm border border-white/5">
                                        ...
                                        >
                                            <div
                                                className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${autoGainControl ? "left-6" : "left-1"}`}
                                            />
                                        </button>
                                    </div>
                                </div>
                                            ...
                                            <div className="space-y-3 p-3 bg-black/20 rounded-sm border border-white/5">
                                                ...
                                                <div>
                                                    <div className="flex justify-between text-[10px] mb-1">
                                                        ...
                                                        <span className="opacity-40">
                                                            {ttsCustomParams.exaggeration.toFixed(
                                                                1,
                                                            )}
                                                        </span>
                                                    </div>
                                                ...
                                                <div>
                                                    <div className="flex justify-between text-[10px] mb-1">
                                                        ...
                                                        <span className="opacity-40">
                                                            {ttsCustomParams.stability.toFixed(
                                                                1,
                                                            )}
                                                        </span>
                                                    </div>
                                                ...
                                                <div>
                                                    <div className="flex justify-between text-[10px] mb-1">
                                                        ...
                                                        <span className="opacity-40">
                                                            {ttsCustomParams.sentence_silence.toFixed(
                                                                1,
                                                            )}
                                                            s
                                                        </span>
                                                    ...
                                                    <input
                                                        ...
                                                        step="0.1"
                                                        value={
                                                            ttsCustomParams.sentence_silence
                                                        }
                                                        onChange={(e) =>
                                                            setTtsCustomParams(
                                                                (p) => ({
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-4">
                                    <div className="flex-1 space-y-1">
                                        <label className="text-[10px] opacity-60 block">
                                            Accent Color
                                        </label>
                                ...
                                <div>
                                    <label className="text-[10px] opacity-60 mb-2 block">
                                        Typography
                                    </label>
                                    <div className="flex gap-2">
                                        {[
                    ...
                    <div className="w-full max-w-sm bg-zinc-900 border border-red-500/30 rounded-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="p-5 space-y-4">
                            <p className="text-xs opacity-70 leading-relaxed">
                                This action will clear any stored API
                                credentials and force a re-authentication. The
                                page will reload, and you will need to select or
                                provide your API key again.
                            </p>
                    ...
                    <div className="w-full max-w-2xl bg-[#18181b] border border-white/10 rounded-lg shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-4 flex-1 overflow-hidden flex flex-col gap-2">
                            <p className="text-[11px] opacity-50">
                                This text is stored in an encrypted browser
                                database. It is injected into every prompt for
                                this context pool.
                            </p>
                            <textarea
                                value={tempMemoryEdit}
                                onChange={(e) =>
                                    setTempMemoryEdit(e.target.value)
                                }
