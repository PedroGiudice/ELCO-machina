...
#[tauri::command]
async fn stop_sidecar(state: tauri::State<'_, SidecarManager>) -> Result<(), String> {
...
#[tauri::command]
async fn sidecar_status(state: tauri::State<'_, SidecarManager>) -> Result<bool, String> {
...
async fn monitor_sidecar(app: tauri::AppHandle, manager: Arc<SidecarManager>) {
...
pub fn run() {
    ...
    let manager_arc = Arc::new(SidecarManager {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Iniciar loop de monitoramento do sidecar
            let app_handle = app.handle().clone();
            tauri::async_runtime::spawn(async move {
                monitor_sidecar(app_handle, monitor_manager).await;
            });

            Ok(())
        })
        .on_window_event(|window, event| {
            // Cleanup ao fechar janela principal
